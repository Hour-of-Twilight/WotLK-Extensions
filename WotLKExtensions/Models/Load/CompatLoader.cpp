...
#include "Models/Load/NativeLoad.h"
...
namespace
{
     ...
     *
     * When the native MD21 reader is compiled in (ModernM2::kEnabled) and the resident buffer is a
    ...
    {
        // The first model of the session is the earliest point the archive set is known mounted.
        if (ModernM2::kEnabled)
            ModernM2::Fdid::BeginWarm();

        ModernM2::Live::OnModelLoadPre(model);

        if (ModernM2::kEnabled && ModernM2::Reader::IsModernContainer(model))
            return ModernM2::Reader::NativeLoad(model);
        return g_origM2Init(model);
    }
    ...
    {
        ModernM2::Live::OnSkinFinalize(model);
        ...
        __try
        {
            ...
            auto* skin = static_cast<ModernM2::Client::M2SkinProfile*>(mdl->skin);
...
namespace ModernM2
{
    ...
    {
        HookAttach("M2.Init", m2::kInit, &hkM2Init, &g_origM2Init);
        HookAttach("M2.FinalizeSkin", m2::kFinalizeSkin, &hkFinalizeSkin, &g_origFinalizeSkin);
        HookAttach("M2.BuildBatchMaterial", m2::kBuildBatchMaterial, &hkBuildBatchMaterial,
                  &g_origBuildBatchMaterial);
        return true;
    }
}
