...
#include "Models/Load/M2NativeInternal.h"
...
namespace
{
    using namespace ModernM2::Reader::Detail;
    ...
    void NativeLoadCore(void* model, Outcome& out)
    {
        ...
        if (mdl->flags & 1u)
        {
            ...
            ModernM2::Live::RegisterNativeLoaded(model);
            out.ok = 2;
            return;
        }
        ...
        if (h->version < ModernM2::Live::kSourceVersionMin ||
            h->version > ModernM2::Live::kSourceVersionMax)
        {
        ...
        ModernM2::Client::M2Model(model).SetBuffer(buf, s.bodySize);
        ...
        if (!ModernM2::Native<off::M2_SharedInitializeFn>(off::kSharedInitialize)(model))
        {
        ...
        void* extArr = ModernM2::Native<off::SMemAllocFn>(off::kSMemAlloc)(
...
namespace ModernM2
{
    bool InstallM2Native()
    {
        WLOG_INFO("m2native: native MD21 reader active (source versions %u-%u, direct native fill)",
                  ModernM2::Live::kSourceVersionMin,
                  ModernM2::Live::kSourceVersionMax);
        return true;
    }
}
...
namespace ModernM2::Reader
{
    ...
    bool Enabled() { return ModernM2::kEnabled; }
    ...
    bool IsModernContainer(void* model)
    {
        ...
        return Detail::Rd32(mdl->header) == ModernM2::Format::kMagicMD21;
    }
    ...
    int NativeLoad(void* model)
    {
        ...
        const char* stem = ModernM2::Client::M2Model(model).GetPathStem();
        if (!stem) stem = "(no stem)";

        if (!out.ok)
        {
            g_statFailed.fetch_add(1, std::memory_order_relaxed);
            ModernM2::Live::ForgetNativeLoaded(model); // undo a pre-Initialize registration
