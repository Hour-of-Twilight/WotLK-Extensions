...
// The arena RESERVATION itself lives in Models/Client/M2Arena.cpp -- see its own comment for why.
// This file only owns the DECISION of which allocations to route there: past a
// size threshold, ask the arena for a range; if the arena is disabled/exhausted/failed, fall back to
...
namespace
{
    ...
    bool LargeM2VirtualAllocEnabled()
    {
        static const bool enabled =
            ModernM2::ConfigFlag("HOT_M2_VIRTUAL_ALLOC", "HoT_m2_virtual_alloc.disable");
        return enabled;
    }

    uint32_t VirtualM2AllocThreshold()
    {
        static const uint32_t bytes = ModernM2::ConfigBytesMbKb(
            "HOT_M2_VIRTUAL_ALLOC_THRESHOLD_MB", "HOT_M2_VIRTUAL_ALLOC_THRESHOLD_KB",
    ...
    void __cdecl hkM2BufferFree(void* ptr)
    {
        ...
        if (ours && !alloc.base)
        {
            ModernM2::Arena::Free(alloc.arenaOffset, alloc.arenaSize);
            return;
        }
        ...
        {
            {
                ...
                if (void* ptr = ModernM2::Arena::Alloc(size, &offset, &allocSize))
                {
                    {
                        std::lock_guard<std::mutex> lock(g_virtualM2AllocMutex);
                    ...
                    if (size >= 8u * 1024u * 1024u) ModernM2::Arena::LogAddressSpace("m2-arena");
                    return ptr;
                }
            }
            ...
            if (void* standalone = TryVirtualM2Alloc(size))
            {
                ...
                if (size >= 8u * 1024u * 1024u)
                    ModernM2::Arena::LogAddressSpace("m2-virtual");
                return standalone;
            }
            if (size >= 8u * 1024u * 1024u)
                ModernM2::Arena::LogAddressSpace("m2-virtual-failed");
...
namespace ModernM2
{
    ...
    {
        HookAttach("M2.BufferAlloc", m2::kBufferAlloc, &hkM2BufferAlloc, &g_origM2BufferAlloc);
        HookAttach("M2.BufferFree", m2::kBufferFree, &hkM2BufferFree, &g_origM2BufferFree);
        return true;
    }
}
