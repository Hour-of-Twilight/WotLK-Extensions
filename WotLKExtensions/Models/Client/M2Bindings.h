 ...
 *
 * A module writes ModernM2::Client::ResolveTexture(h) instead of casting an address. The wrappers
 * are zero-overhead inline typed calls.
 */
namespace ModernM2::Client
{
...
#pragma pack(push, 1)
    struct M2SkinProfile
    {
        ...
        ModernM2::Format::M2SkinSection* submeshes;   // 0x20
        uint32_t                          batchCount;   // 0x24  (texunits)
        ModernM2::Format::M2Batch*      batches;      // 0x28
...
#pragma pack(pop)
     ...
     * `ModernM2::Gx::Device9` already established for this SDK layer.
     */
    class M2Model
    {
    public:
         ...
         * file offsets.
         */
        ModernM2::Format::M2Header* GetHeader() const
        {
            return reinterpret_cast<ModernM2::Format::M2Header*>(GetFileBuffer());
        }
