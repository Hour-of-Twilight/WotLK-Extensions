...
namespace
{
    ...
    using ModernM2::Client::M2SkinProfile;
    using ModernM2::Format::M2Header;
    using ModernM2::Format::M2SkinSection;
    ...
    {
        return ModernM2::Gx::Vtbl<off::Gx_BufLockFn>(device, static_cast<unsigned>(off::kGxVtblBufLock / sizeof(void*)))(device, buffer);
    }
    ...
    {
        ModernM2::Gx::Vtbl<off::Gx_BufUnlockFn>(device, static_cast<unsigned>(off::kGxVtblBufUnlock / sizeof(void*)))(device, buffer, 0);
        *At<uint8_t>(buffer, off::kOffGxBufBuilt) = 1;
        ModernM2::Native<off::Gx_PrimIndexPtrFn>(off::kPrimIndexPtr)(device, buffer);
    }
    ...
    {
        void* device = ModernM2::Gx::RawGraphicsDevice();
...
namespace ModernM2::WideIndices
{
     ...
     *
     * The section is read off the batch record rather than from the draw context own copy: the
     * context is reused across every batch in a pass and the batch entry refreshes that copy as its
     * very first act, so a hook on its entry still sees the batch that already ran.
     *
     * The caller saves and restores around the original rather than simply assigning, because nothing
     * promises a batch draw never nests, and a stale section outliving its batch would widen someone
     * else.
     */
    DrawBatchScope OnDrawBatchEnter(void* ctx)
    {
        const DrawBatchScope saved{ g_drawSection, g_drawSkin };
    ...
    {
        g_drawSection = static_cast<const M2SkinSection*>(saved.section);
        g_drawSkin    = static_cast<const M2SkinProfile*>(saved.skin);
    }

    /**
     * @brief Widens a picking range the scene computed from a submesh 16-bit start.
     *
     * The scene ray-versus-geometry pass hands the triangle test a [begin, end) pair it computed
     * from the submesh 16-bit start, so a widened submesh is tested 65536 indices too early and the
     * test walks off its scratch window. Every wide skin is noted when its index buffer is filled; a
     * pair that lands inside a noted skin triangle array and matches a submesh low-16 start and
     * count is remapped to the widened start.
     */
    void RemapHitTestRange(uint16_t** indexBegin, uint16_t** indexEnd)
    {
        if (!indexBegin || !indexEnd || !*indexBegin || *indexEnd <= *indexBegin) return;
    ...
    bool Install()
    {
        HookAttach("M2SetModelIndices", off::kSetModelIndices, &hkSetModelIndices, &g_origSetModelIndices);
        HookAttach("M2SharedSetIndices", off::kSharedSetIndices, &hkSharedSetIndices, &g_origSharedSetIndices);
        HookAttach("GxDeviceDraw", gxoff::kGxDeviceDraw, &hkDeviceDraw, &g_origDeviceDraw);
        HookAttach("M2SharedSetVertices", off::kSharedSetVertices, &hkSharedSetVertices, &g_origSharedSetVertices);
