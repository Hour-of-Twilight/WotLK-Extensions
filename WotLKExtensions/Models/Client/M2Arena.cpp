// Dedicated M2 buffer arena: a reserved 32-bit VA region for large model buffers.
// Ported from WarcraftXL (src/client/CM2Shared/Memory.cpp).
...
// Only the arena RESERVATION lives here (done as early as the DLL runs, before world loading
// fragments the 32-bit VA space). The allocator detours that route large M2 buffers into it (and the
// standalone-VirtualAlloc fallback) live in Models/Load/M2Memory.cpp.
...
namespace
{
    ...
    {
        // One VirtualQuery per region over the whole address space, so snapshot rather than sample.
        // A failed allocation always reports: that is the case this exists to explain.
        static long long s_lastWalk = 0;
        const bool urgent = reason && std::strstr(reason, "failed") != nullptr;
        const long long now = ModernM2::TickNow();
        if (!urgent && s_lastWalk && ModernM2::TickMs(s_lastWalk, now) < 5000.0)
            return;
        s_lastWalk = now;
    ...
    uint32_t M2ArenaSizeMb()
    {
        static const uint32_t mb =
            ModernM2::ConfigU32("HOT_M2_ARENA_MB", kDefaultM2ArenaSizeMb, 64, 2048);
        return mb;
    }

    bool M2ArenaEnabled()
    {
        static const bool enabled = ModernM2::ConfigFlag("HOT_M2_ARENA", "HoT_m2_arena.disable");
        return enabled;
    }
...
namespace ModernM2::Arena
{
	...
	/**
	 * @brief Early reservation of the large-M2 arena before world loading fragments the VA space.
	 *
	 * Grabs the contiguous 32-bit reservation early so a later large model buffer finds room;
	 * deferring it to the first allocation would race the client's own world-load allocations for the
	 * same scarce address space. Best-effort: a disabled arena or a failed reserve is not fatal (large
	 * buffers then fall back to a standalone VirtualAlloc or the native allocator).
	 */
	void Reserve()
	{
		std::lock_guard<std::mutex> lock(g_arenaMutex);
