// Ported from WarcraftXL (scripts/modern-m2/src/render/M2Draw.cpp).
...
// The DrawIndexedPrimitive slot is already owned by Rendering/MSDF/D3D.cpp, which swaps it on every
// device create. Rather than a second vtable swap, this rides that one: D3D::SetDrawIndexedPrimitiveFilter
// installs a filter that gets the call before the original and may rewrite its arguments or serve it
// itself, which is the whole of what the modern-M2 draw path needs the slot for.
//
// 0x008203B0 (the M2 batch draw) likewise carries exactly one detour, so the wide-index draw scope
// (Models/Client/WideIndices.cpp) rides this file's detour instead of installing its own.
...
namespace
{
	...
	namespace m2off = ModernM2::Offsets::M2;
	namespace gx = ModernM2::Gx;
	...
	// Ribbon multi-texture: set true around a >= 3 layer ribbon's single native pass so the DIP filter
	...
	/**
	 * @brief Detours the M2 batch draw, recording the drawing context so the DIP filter can name it.
	...
	void __fastcall hkDrawBatch(void* ctx, void* edx)
	{
		...
		g_curDrawCtx = ctx;
		const ModernM2::WideIndices::DrawBatchScope wide = ModernM2::WideIndices::OnDrawBatchEnter(ctx);

		g_origDrawBatch(ctx, edx);

		ModernM2::WideIndices::OnDrawBatchLeave(wide);
	 ...
	 * the index window.
	 * By the time DIP is called, drawCtx+0x90 points at the copied M2SkinSection for this batch, so the
	 * filter can restore the full 32-bit StartIndex without touching normal legacy sections.
	 */
	unsigned ExpandM2StartIndex(unsigned startIndex) noexcept
	{
		...
		__try
		{
			...
			const auto* sec = static_cast<const ModernM2::Format::M2SkinSection*>(ctx->section);
			if (!sec || sec->level == 0)
				return startIndex;
			if ((startIndex & 0xFFFFu) != sec->indexStart)
				return startIndex;
	 ...
	 */
	HRESULT DrawRibbonMultiTexture(D3D::DrawIndexedPrimitive_t original, IDirect3DDevice9* dev,
	    D3DPRIMITIVETYPE pt, INT bv, UINT mi, UINT nv, UINT si, UINT pc)
	{
		DWORD s[4][4];
		...
		const HRESULT r = original(dev, pt, bv, mi, nv, si, pc);
	...
	/**
	 * @brief Detours the ribbon emitter draw, optionally folding layers into one pass.
	 *
	 * For a three-or-more-layer ribbon, pre-binds s1/s2, flags the draw so the DIP filter folds the
	 * layers, and clamps the layer count to 1 so the native draw runs exactly one pass. Otherwise the
	 * draw runs untouched.
	 */
	int __fastcall hkRibbonDraw(void* self, void* edx, void* stateBlock)
	{
		...
		if (emitter)
		{
			__try
			{
				...
				bool useMulti = false;
				ModernM2::Live::OnRibbonDraw(layers, &useMulti);
	...
	/**
	 * @brief The DrawIndexedPrimitive filter MSDF's device hook calls in place of the original.
	 *
	 * Folds multi-texture ribbons into one combined pass, and otherwise runs the native draw with the
	 * 32-bit M2 start index restored.
	 */
	HRESULT STDMETHODCALLTYPE M2DrawIndexedPrimitiveFilter(D3D::DrawIndexedPrimitive_t original,
	    IDirect3DDevice9* dev, D3DPRIMITIVETYPE pt, INT bv, UINT mi, UINT nv, UINT si, UINT pc)
	{
		if (g_ribbonModern)
			return DrawRibbonMultiTexture(original, dev, pt, bv, mi, nv, si, pc);
		return original(dev, pt, bv, mi, nv, ExpandM2StartIndex(si), pc);
	}
}
...
namespace ModernM2
{
	bool InstallM2Draw()
	{
		D3D::SetDrawIndexedPrimitiveFilter(&M2DrawIndexedPrimitiveFilter);

		HookAttach("M2.DrawBatch", off::kDrawTriangleBatch, &hkDrawBatch, &g_origDrawBatch);
		HookAttach("M2.RibbonDraw", m2off::kRibbonDraw, &hkRibbonDraw, &g_origRibbonDraw);

		WLOG_INFO("m2-draw: batch/ribbon detours installed, DrawIndexedPrimitive filter armed");
		return true;
	}
}
