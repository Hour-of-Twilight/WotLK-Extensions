...
typedef enum EDiscordResult(DISCORD_API* DiscordCreateProc)(DiscordVersion version,
    struct DiscordCreateParams* params,
    struct IDiscordCore** result);

static HMODULE s_discordModule = nullptr;
static DiscordCreateProc s_DiscordCreate = nullptr;

static bool LoadDiscordSDK()
{
    if (s_discordModule)
        return s_DiscordCreate != nullptr;
    s_discordModule = LoadLibraryA("discord_game_sdk.dll");
    if (!s_discordModule)
        return false;
    s_DiscordCreate = reinterpret_cast<DiscordCreateProc>(
        GetProcAddress(s_discordModule, "DiscordCreate"));
    return s_DiscordCreate != nullptr;
}
...
Result Core::Create(ClientId clientId, std::uint64_t flags, Core** instance)
{
    ...
    auto result = s_DiscordCreate(DISCORD_VERSION, &params, &((*instance)->internal_));
