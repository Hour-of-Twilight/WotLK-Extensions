...
//#ifdef _DEBUG
                char buf[2048];
                _snprintf_s(buf, sizeof(buf), _TRUNCATE, "[ClientExtensions] %s Shader Error: %s\n",
...
//#endif
    ...
    const ShaderEntry* s_shaders = s_shaders_engine;
    const size_t s_shaders_count = std::size(s_shaders_engine);
